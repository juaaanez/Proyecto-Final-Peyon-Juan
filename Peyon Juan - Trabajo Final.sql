...
insert into ZonaDelivery (id,zona) values (1, 'norte');
insert into ZonaDelivery (id,zona) values (2, 'sur');
insert into ZonaDelivery (id,zona) values (3, 'este');
insert into ZonaDelivery (id,zona) values (4, 'oeste');

-- Creacion vistas

create view precios as
select Tipo, Precio from productos;

create view deliverys as
select tipo from Delivery order by tipo asc;

create view tiposproductos as
select tipo from Productos order by tipo desc;
...
-- Se crearan dos tablas nuevas (TipoAplicacion y Trabajadores ) y se trabajará sobre ellas
